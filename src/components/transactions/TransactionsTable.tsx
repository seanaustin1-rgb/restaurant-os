...
export function TransactionsTable({ rows }: { rows: TxnRow[] }) {
  ...
  const merchant = (r: TxnRow) => (
    <>
      <span className="text-[#E6E8E4]">{r.merchantName ?? r.description ?? "—"}</span>
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="border-b border-line/60 last:border-0">
              ...
              <td className="tnum px-4 py-2 text-right text-[#E6E8E4]">{money2(r.amount)}</td>
        ...
        {rows.map((r) => (
          <div key={r.id} className="p-3">
            <div className="flex items-start justify-between gap-2">
              ...
              <span className="tnum shrink-0 text-[#E6E8E4]">{money2(r.amount)}</span>
            </div>
