import { auth } from "@clerk/nextjs/server";
import { redirect } from "next/navigation";
import { prisma } from "@/lib/prisma";
import { loadPaymentWatch } from "@/lib/modules/payment-watch";
import { PaymentWatchModule } from "@/components/modules/PaymentWatchModule";

export default async function PaymentWatchPage() {
  const { userId } = await auth();
  if (!userId) redirect("/sign-in");

  const role = await prisma.userRestaurantRole.findFirst({
    where: { clerkUserId: userId },
    select: { restaurantId: true, restaurant: { select: { name: true } } },
  });

  const data = role ? await loadPaymentWatch(role.restaurantId) : null;

  return (
    <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6 sm:py-10">
      <div>
        <h1 className="font-display text-2xl text-copper-soft">Payment Watch</h1>
        <p className="mt-1 text-sm text-muted">
          {role?.restaurant?.name ?? "Your business"}
          {data?.hasData ? ` â€” ${data.windowLabel}` : ""} Â· likely double-pays and off-norm
          charges, for review.
        </p>
      </div>
      {data?.hasData ? (
        <PaymentWatchModule data={data} />
      ) : (
        <p className="rounded-lg border border-dashed border-line p-8 text-center text-sm text-muted">
          No transactions yet. Import a statement to start watching for duplicates.
        </p>
      )}
    </main>
  );
}
