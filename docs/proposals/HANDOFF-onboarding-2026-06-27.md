...
> **Update (resume session): §5 decisions resolved.** A1 → merge docs as
> spec-of-record (phase-2 engine = separate issue, A2). C5 → onboarding reordered
> Industry → Name+scale → Sector Qs → Data source. B3/B4 → seed splits
> re-grounded in industry benchmarks; **real-revenue basis** adopted for
> contractor, brokerage, and the VR-manager case (pass-throughs excluded from the
> sum-to-100). D9/D10 → invite + owner-approval access flow approved and specced
> (§4.6). **New (user-driven):** real-estate/VR onboarding now imports the full
> **roster** (agents / properties) instead of asking a count, and each entity
> gets a **red/amber/green morning triage** read (design-copy spec §6). The two
> proposal docs are now spec-of-record; this handoff stays as the historical
> snapshot. Remaining open: B3 fine-tuning of specific sector numbers if desired;
> E11 signed-in screenshots.
