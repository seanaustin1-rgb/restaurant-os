    ...
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3 border-b border-line pb-4">
        <div>
          ...
          <h2 className="font-display text-3xl text-[#E6E8E4]">{f.name || "Your rental portfolio"}</h2>
      ...
      <div className="mt-6 rounded-lg border border-line bg-surface px-4 py-3 text-[11px] leading-relaxed text-muted">
        Source pipe: <span className="text-[#E6E8E4]">{r.pmsLabel}</span>. {r.pmsNote}
      </div>

      <div className="mt-8 rounded-xl border border-line bg-surface px-5 py-5 text-center">
        <div className="font-display text-xl text-[#E6E8E4]">Want the per-property picture?</div>
...
function BreakEvenTile({ r }: { r: VacationRentalEstimateResult }) {
  return (
    <Tile title="Break-even occupancy" icon={<Wallet size={12} className="text-copper-soft" />} badge={YOURS} explainer={EXPLAIN.breakeven}>
      <div className="flex items-baseline gap-2">
        <span className="tnum text-4xl text-[#E6E8E4]">{r.breakEvenOccupancyPct != null ? pct(r.breakEvenOccupancyPct, 0) : "—"}</span>
...
function SelfManageTile({ r }: { r: VacationRentalEstimateResult }) {
  return (
    <Tile title="Self-managed vs. managed" icon={<PiggyBank size={12} className="text-copper-soft" />} badge={YOURS} explainer={EXPLAIN.selfmanage}>
      ...
      <p className="mt-3 text-[13px] leading-relaxed text-[#CFD2CC]">
        ...
        <span className="tnum text-health-green">{pct(r.selfManagedOwnerProceedsPct, 0)}</span> — about <span className="tnum text-[#E6E8E4]">{money(r.managementFeeMonthlyCost * 12)}</span> a year. Worth it only if your manager earns it back in occupancy.
      </p>
    </Tile>
  );
}
